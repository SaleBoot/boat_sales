...
function defaultConfigTabs(profile, variants) {
  ...
  if (isUnmanned) {
    ...
    const woodVeneers = [
      ...
      { id: 'wood-ash', name: '木饰面 · Ash 缎面白蜡木（选配）', description: '浅色顺纹白蜡木，通透清爽，适合现代北欧风格舱室', priceDelta: 8, sortOrder: 11 },
...
function normalizeTwinConfig(value) {
  ...
  } else if (raw.power && typeof raw.power === 'object') {
    power = { id: String(raw.power.id || ''), name: String(raw.power.name || ''), description: String(raw.power.description || '') };
  }
